...
if "use_recorded_voice" not in st.session_state:
    st.session_state.use_recorded_voice = False  # إذا رفع صوت مسجل يستخدمه للردود
if "recorded_voice_path" not in st.session_state:
    st.session_state.recorded_voice_path = None

# ======================= إعداد Gemini بموديل 3.5 مع fallback =======================
def setup_gemini():
    ...
    # قائمة النماذج المطلوبة (الأول المفضل 3.5)
    models_to_try = [
        'gemini-3.5-flash',          # الذي تريده أنت
        'models/gemini-3.5-flash',   # محاولة بالبادئة
        'gemini-1.5-flash',          # البديل الأحدث
        'models/gemini-1.5-flash',
        'gemini-pro'                 # القديم لكن يعمل
    ]
...
def text_to_speech_tts(text, lang='ar'):
    """تحويل النص إلى صوت باستخدام gTTS (صوت افتراضي ليس صوتك)"""
    try:
...
# ======================= تأثير تحريك الشفاه (محاكاة بسيطة) =======================
def animate_avatar(image_path, duration=2):
    """تأثير وميض وتحريك بسيط لمحاكاة حركة الشفاه"""
    if not os.path.exists(image_path):
        return
    # عرض الصورة مع تأثير وميض عبر إعادة عرضها بسرعة
    placeholder = st.empty()
    for i in range(3):  # 3 ومضات
...
with tab1:
    ...
    with col_img:
        if os.path.exists(st.session_state.current_avatar):
            ...
            st.warning("الصورة غير موجودة، تأكد من رفعها.")
        if st.button("🔄 اختبار حركة الشفاه"):
            animate_avatar(st.session_state.current_avatar, duration=1.5)
    
    with col_chat:
        st.markdown("### 🧠 مرحباً! أنا سعيد الذكي، اسألني عن أي شيء")
        ...
        if user_question:
            ...
            if st.session_state.voice_enabled:
                ...
                if st.session_state.use_recorded_voice and st.session_state.recorded_voice_path and os.path.exists(st.session_state.recorded_voice_path):
                    # استخدام الصوت المسجل (نفس المقطع لكل رد، ليس مثالياً لكنه يحقق طلبك)
                    st.audio(st.session_state.recorded_voice_path, format='audio/mp3')
                else:
                    # توليد صوت من النص بصوت افتراضي
                    audio_file = text_to_speech_tts(ai_reply, lang='ar')
                    if audio_file:
                        st.audio(audio_file, format='audio/mp3')
                        os.unlink(audio_file)  # تنظيف
                    else:
